...
const needsHuman = [
  ["CLIENT B", "SURFACE", "Direct tone diverges materially from approved client style."],
  ["CLIENT A", "REQUIRE APPROVAL", "Added a 4-hour response-time commitment, creating a new commercial obligation."],
]
      ...
      <div className="mx-auto max-w-6xl px-5 py-20 md:py-28">
        ...
        <div className="mt-12 overflow-hidden rounded-sm border border-border bg-card shadow-[0_1px_0_var(--border),0_12px_32px_-16px_rgb(0_0_0/0.18)]">
          ...
          <div className="grid gap-px bg-border md:grid-cols-2">
            ...
            <div className="bg-card p-6"><p className={`font-mono text-xs uppercase tracking-widest ${done ? "text-muted-foreground" : "text-primary"}`}>{done ? "Resolved" : "Human judgment required"}</p><ul className="mt-5 flex flex-col gap-3">{needsHuman.map(([title, decision, detail]) => <li key={title} className={`rounded-sm border p-4 ${done ? "border-border" : "border-destructive/40 bg-destructive/5"}`}><div className="flex items-center gap-2">{done ? <Check className="size-4 text-primary" aria-hidden="true" /> : <AlertTriangle className="size-4 text-destructive" aria-hidden="true" />}<span className="font-mono text-sm font-semibold">{title}</span><span className="ml-auto font-mono text-[10px] uppercase tracking-wider text-primary">{done ? "AUTHORIZED" : decision}</span></div><p className={`mt-2 text-sm leading-relaxed ${done ? "text-muted-foreground line-through" : "text-foreground"}`}>{detail}</p></li>)}</ul></div>
          </div>
